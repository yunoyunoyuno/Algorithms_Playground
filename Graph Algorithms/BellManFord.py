...
def BellmanFord(G,s):
    E = G.edges();v = G.size();p = [None for _ in range(v)];
    d = [1e5 for _ in range(v)]; d[s-1] = 0;
    for i in range(1,v):
        for e in E:
            ui = e[0]-1; vi = e[1]-1; wi = e[2];
            if(d[ui] + wi < d[vi]): d[vi] = d[ui] + wi;p[vi] = ui + 1;
    for i in range(1,v):
        for e in E:
            ui = e[0]-1; vi = e[1]-1; wi = e[2];
            if(d[ui] + wi < d[vi]): return None;
    s = sum(d); ANS = [];
    for i,n in enumerate(p):
        ans = "";
        if(n != None): ans = f'{p[i]} -> {i+1}'; ANS.append(ans);
    print("Parent node from that node must be");
    for e in ANS: print(e);
        
    

V = 5; E = [(1,2,2),(2,3,2),(3,4,4),(3,5,1),(5,2,-1)];
G = Graph(V,E);
BellmanFord(G,2);
